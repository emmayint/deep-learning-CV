...
router.get("/", function(req, res, next) {
  if (req.isAuthenticated()) {
    ...
    db.query(
      "SELECT * FROM experiments WHERE users_id= " + user.user_id + "",
...
router.post("/", upload.array("expImage", 40), function(req, res, next) {
  ...
  userKey = req.body.action_id;
  userValue = req.body.action_name;
  ...
  db.query(
    "INSERT INTO experiments (users_id, exp_title, exp_birth_date, exp_misc_data) VALUES (?, ?, ?, ?)",
    [user.user_id, expTitle, expBirthDate, JSON.stringify(dict)],
    function(error, results, fields) {
      if (error) throw error;
      ...
      db.query(
        ...
        function(error, results, fields) {
          if (error) throw error;
          db.query("SELECT  * FROM experiment_images", function(
            err,
            results2,
            field2
          ) {
