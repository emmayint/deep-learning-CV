...
router.post("/:id/deleteImages", function(req, res) {
  try {
    ...
    //     db.query(
    //       "DELETE experiment_images, experiment_cropped_images FROM experiment_images INNER JOIN experiment_cropped_images WHERE experiment_images.id = experiment_cropped_images.exp_img_id AND experiment_images.ID IN (" +
    //         images +
    //         ");",
    //       function(err, result) {}
    //     );
    //   }
    db.query(
      "DELETE FROM experiment_images WHERE ID IN (" + images + ");",
      function(err, result) {}
    );
  } catch (err) {
    console.log(err);
      ...
      function(err, result) {
        for (let i = 0; i < result.length; i++) {
          ...
          if (typeof rect !== "undefined") {
            db.query(
              'UPDATE experiment_images SET rect="' +
                resRect +
                '" WHERE ID = ' +
                result[i].id +
                ";",
                ...
                probe(ImageUrl + result[i].exp_images, function(err, result1) {
                  ...
                  if (rectDataNew.length > 2) {
                    ...
                    let topVal = Math.round(rectDataNew[0] / ratio),
                      widthVal = Math.round(
                        (rectDataNew[1] - rectDataNew[3]) / ratio
                      ),
                      heightVal = Math.round(
                        (rectDataNew[2] - rectDataNew[0]) / ratio
                      ),
                      leftVal = Math.round(rectDataNew[3] / ratio);

                    let tempFilename =
                      Math.floor(Math.random() * 1000000000) + ".jpg";

                    download(
                      ImageUrl + result[i].exp_images,
                      tempFilename,
                      function() {
                        sharp(tempFilename)
                          .extract({
                          ...
                          .then(function(new_file_info) {
                            ...
                            db.query(
                              "INSERT INTO experiment_cropped_images (exp_id, user_id, exp_img_id, exp_crop_img, created_at, rect, exp_label_name) VALUES (?, ?, ?, ?, ?, ?, ?)",
                              [
                                result[i].exp_id,
                                result[i].user_id,
                                result[i].id,
...
router.post("/:id", upload.array("expImage", 40), function(req, res, next) {
  ...
  db.query(
    "SELECT * FROM experiments WHERE users_id = " +
      user.user_id +
      " AND exp_id=" +
      id +
      " ",
    function(error, results, fields) {
      ...
      db.query(
        "SELECT * FROM experiment_images WHERE exp_id=" + id + "",
        function(err, results2, field2) {
          if (error) throw error;
