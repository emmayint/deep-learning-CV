...
router.get("/:id", function(req, res, next) {
  if (req.isAuthenticated()) {
    ...
    db.query(
      "SELECT e.exp_id, e.img_dir, e.label FROM experiment_images e, models m  WHERE e.exp_type='T' AND e.exp_id=m.exp_id AND e.user_id = " + user.user_id + 
      " AND e.exp_id = " +
        id +
        ";",
      function(error, results, fields) {
        if (error) throw error;
            db.query(
                "SELECT imgs01,imgs10 FROM models WHERE user_id = " + user.user_id + 
                " AND exp_id = " +
                  id +
                  ";",
